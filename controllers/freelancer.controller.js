...
export const getTopFreelancersByService = async (req, res, next) => {
  try {
    ...
    const topFreelancers = await Contract.findAll({
      ...
      attributes: [
        [sequelize.col("contracts.freelancer_id"), "freelancer_id"],
        [
          sequelize.fn("COUNT", sequelize.col("contracts.freelancer_id")),
          "total_completed",
        ],
      ],
      group: ["contracts.freelancer_id", "freelancer.id", "service.id"],
      order: [[sequelize.literal("total_completed"), "DESC"]],
    });
    

    res.status(200).json({
      success: true,
      message: "Top freelancers by service",
      data: topFreelancers,
    });
  } catch (err) {
    next(err);
